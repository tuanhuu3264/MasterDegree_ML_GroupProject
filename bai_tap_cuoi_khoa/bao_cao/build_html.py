...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .cover {{ text-align:center; padding-top:20px; color:#111; }}
  .cover .univ {{ font-size:15pt; font-weight:700; letter-spacing:0.4px; line-height:1.5; color:#111; }}
  .cover .cover-title {{ font-size:17.5pt; margin:140px 36px 0; color:#111; line-height:1.4; }}
  .cover .cover-vi {{ font-size:11pt; font-style:italic; color:#555; margin:12px 40px 0; }}
  .cover .byline {{ font-size:13pt; margin-top:80px; color:#111; }}
  .cover .authors {{ margin-top:56px; font-size:12.5pt; line-height:2.05; color:#111; }}
  .cover .supervisor {{ margin-top:30px; font-size:12pt; color:#111; }}
  .cover .submit {{ margin-top:96px; font-size:11.5pt; color:#111; line-height:1.5; }}
  .cover .copyr {{ margin-top:44px; font-size:11.5pt; color:#111; }}
  ...
  strong {{ color:#14243d; }}
  .toc {{ margin-top:10px; }}
  .toc > ol {{ font-size:11.5pt; line-height:1.5; padding-left:24px; }}
  .toc > ol > li {{ margin:7px 0; font-weight:700; color:#14243d; }}
  .toc ul {{ list-style:none; padding-left:16px; margin:3px 0 5px; }}
  .toc ul li {{ margin:2px 0; font-size:10.2pt; font-weight:400; color:#333; }}
  .toc-extra {{ margin-top:12px; padding-top:8px; border-top:1px dotted #b8bfc9;
                font-size:11pt; font-weight:700; color:#14243d; }}
  .kw {{ font-size:10pt; margin-top:8px; }}
</style></head>
<body>

<!-- ================= TRANG BIA (mau FPT University) ================= -->
<div class="cover">
  <div class="univ">MINISTRY OF EDUCATION AND TRAINING</div>
  <div class="univ">FPT UNIVERSITY</div>

  <div class="cover-title">Predictive Maintenance of CNC Machines<br>under Distribution Shift</div>
  <div class="cover-vi">Dự đoán hỏng hóc thiết bị CNC trong điều kiện dịch chuyển phân phối dữ liệu</div>

  <div class="byline">by</div>

  <div class="authors">
    25MS23315 &nbsp;·&nbsp; Trịnh Hữu Tuấn<br>
    25MS23323 &nbsp;·&nbsp; Hoàng Phi Hải<br>
    25MS23327 &nbsp;·&nbsp; Đỗ Hoàng Tỷ Phú<br>
    25MS23328 &nbsp;·&nbsp; Lê Lâm Vĩnh
  </div>

  <div class="supervisor">Supervisor: Dr. Cao Tiến Dũng</div>

  <div class="submit">A group project submitted in conformity with the requirements<br>
  for the fulfillment of the course &ldquo;Machine Learning&rdquo;</div>

  <div class="copyr">&copy; Copyright by Group of 4 students &nbsp;·&nbsp; 2026</div>
</div>
...
<em>dịch chuyển hiệp biến</em> (covariate shift) [4] và đề xuất bốn đặc trưng "khoảng cách tới biên" neo trên các ngưỡng
của cơ chế sinh lỗi được <em>khôi phục từ chính Dây chuyền A</em> (HDF, PWF, OSF, TWF) — theo tinh thần bộ dữ liệu
AI4I 2020 [1]. Vì các ngưỡng này là thuộc tính cơ chế bất biến giữa hai dây chuyền, ranh giới quyết định học trên
Dây chuyền A chuyển giao trực tiếp sang Dây chuyền B (PSI ≈ 0; Drift-AUC của đặc trưng biên = 0,53). Các kỹ thuật
hiệu chỉnh trọng số theo tỉ lệ mật độ [2], hiệu chỉnh ngưỡng và học kết hợp được áp dụng như các lớp tinh chỉnh,
toàn bộ được lựa chọn thông qua kiểm định có trọng số quan trọng (Importance-Weighted Validation) [3] nhằm bảo đảm
không rò rỉ nhãn tập kiểm thử. Mô hình cuối cùng (học kết hợp bỏ phiếu giữa Random Forest, ExtraTrees và XGBoost)
đạt F1 = 0,783 trên Dây chuyền B, cải thiện 3,4 lần so với mô hình cơ sở. Phân tích cho thấy con số này chạm trần
hiệu năng của dữ liệu: khoảng 25% ca hỏng là nhiễu ngẫu nhiên không thể học được.</p>
...
<ol>
<li>Giới thiệu</li>
<li>Dữ liệu và phát biểu bài toán
   <ul>
   <li>2.1 Mô tả dữ liệu</li>
   <li>2.2 Chẩn đoán loại dịch chuyển phân phối</li>
   <li>2.3 Lựa chọn độ đo đánh giá</li>
   </ul></li>
<li>Phân tích khám phá dữ liệu</li>
<li>Tổng hợp các quyết định thiết kế</li>
<li>Phương pháp đề xuất và luận cứ
   <ul>
   <li>5.1 Độ đo đánh giá</li>
   <li>5.2 Chiến lược chia dữ liệu</li>
   <li>5.3 Thiết kế đặc trưng (khôi phục ngưỡng luật sinh nhãn từ Dây chuyền A)</li>
   <li>5.4 Cơ chế ngưỡng và tính bất biến hạng</li>
   <li>5.5 Lựa chọn họ mô hình</li>
   <li>5.6 Hiệu chỉnh dịch chuyển bằng trọng số tỉ lệ mật độ</li>
   <li>5.7 Kiểm định có trọng số quan trọng (IWV)</li>
   <li>5.8 Học kết hợp</li>
   <li>5.9 Ngưỡng quyết định</li>
   </ul></li>
<li>Thực nghiệm và kết quả
   <ul>
   <li>6.1 Đánh giá độ tin cậy bằng bootstrap</li>
   </ul></li>
...
<div class="toc-extra">Phụ lục. &nbsp;Kiểm soát rò rỉ dữ liệu<br>
...
<p>Việc xác định đúng loại dịch chuyển là điều kiện tiên quyết, bởi mỗi loại đòi hỏi một chiến lược xử lý khác nhau [4].
...
với mục tiêu vận hành và được khuyến nghị cho dữ liệu mất cân bằng mạnh [10]. Báo cáo trình bày đồng thời ba độ đo: F1 là độ đo so sánh chính, AUC-PR là tham chiếu về độ
...
<tr><td>Lựa chọn mô hình/ngưỡng bằng IWV</td><td>Bài toán chấm trên B nhưng cấm sử dụng nhãn B</td><td>Lựa chọn theo điểm số trên B (rò rỉ nhãn kiểm thử, đánh giá thiên lệch)</td><td>Cao — cỡ mẫu hiệu dụng thấp (24,2%) khiến IWV nhiễu</td></tr>
<tr><td>Học kết hợp: chọn bỏ phiếu thay vì xếp chồng</td><td>Bốn mô hình cơ sở ít tương quan; bỏ phiếu đơn giản, ít quá khớp dịch chuyển</td><td>Xếp chồng — mô hình tổng hợp dồn trọng số vào XGBoost, kém ổn định hơn dưới dịch chuyển</td><td>Thấp — hai phương án gần tương đương (F1-IWV bằng nhau)</td></tr>
</table>
...
các mẫu hỏng lân cận [9]; dưới điều kiện dịch chuyển, vùng lớp hiếm của A không đại diện cho B, khiến mẫu giả làm
mô hình tự tin sai lệch. Thay vào đó, phương pháp sử dụng <code>class_weight='balanced'</code> cho hồi quy logistic và Random
Forest, cùng <code>scale_pos_weight ≈ 12,6</code> cho XGBoost — bù mất cân bằng bằng trọng số hàm mất mát thay vì tạo
dữ liệu giả. Toàn bộ bước chuẩn hóa và mã hóa được đóng gói trong <code>Pipeline</code> của thư viện scikit-learn [11]
và huấn luyện lại trên tập huấn luyện ở mỗi vòng kiểm định chéo nhằm ngăn rò rỉ dữ liệu.</p>
...
<p>Bốn đặc trưng đề xuất, mỗi đặc trưng mã hóa một cơ chế sinh lỗi dưới dạng "khoảng cách tới biên nguy hiểm",
được mô tả trong Bảng 6.</p>
<table>
<caption>Bảng 6. Bốn đặc trưng khoảng cách tới biên và luận cứ dạng hàm.</caption>
<tr><th style="width:18%">Đặc trưng</th><th style="width:34%">Công thức</th><th>Cơ chế và luận cứ dạng hàm</th></tr>
...
<tr><td><code>lech_cong_suat</code></td><td>max(2600 − P, 0) + max(P − 11500, 0)</td>
    <td>Cơ chế PWF (công suất). Tổng hai hàm bản lề tương đương phép tuyển (OR): nguy hiểm khi công suất ra ngoài
    dải an toàn [2600, 11500] W (thiếu hoặc thừa); hai số hạng không thể đồng thời dương.</td></tr>
<tr><td><code>bien_overstrain</code></td><td>mòn × mômen − ngưỡng(loại)<br>{{L: 12800; M: 13900; H: 14500}}</td>
...
<tr><td><code>mon_twf</code></td><td>max(mòn − 244, 0)</td>
    <td>Cơ chế TWF (mòn dao). Hàm bản lề vượt ngưỡng thay dao; là luật mà cấu hình gốc bỏ sót — hệ số tương quan
    với nhãn đạt 0,43, mạnh nhất trong toàn bộ đặc trưng.</td></tr>
</table>
<h4>Khôi phục ngưỡng của luật sinh nhãn từ Dây chuyền A</h4>
<p>Một câu hỏi then chốt là các hằng số ngưỡng ở Bảng 6 được xác định như thế nào. Bộ dữ liệu sinh theo cơ chế
kiểu AI4I [1] tuân theo các luật có ngưỡng xác định; do đó, với mỗi cơ chế, nghiên cứu <em>quét thử ngưỡng chỉ trên
Dây chuyền A</em> và xác định điểm mà xác suất hỏng có điều kiện tăng vọt — đó chính là biên mà luật sử dụng để sinh
nhãn. Toàn bộ quá trình <strong>không sử dụng nhãn của B</strong>; các biên sau đó được kiểm chứng là bất biến khi
chuyển sang B (Bảng 7). Bảng 6b tổng hợp bằng chứng cho từng ngưỡng.</p>
<table>
<caption>Bảng 6b. Căn cứ khôi phục ngưỡng từ Dây chuyền A (không dùng nhãn B).</caption>
<tr><th style="width:12%">Cơ chế</th><th style="width:26%">Ngưỡng chốt</th><th>Bằng chứng trên A</th></tr>
<tr><td>HDF</td><td>ΔT &lt; 8,6 K và tốc độ &lt; 1380</td><td>Giữ nguyên hằng số AI4I: P(hỏng) tại đây đạt 0,84; nới lỏng làm độ chính xác rớt còn ~0,45.</td></tr>
<tr><td>PWF</td><td>công suất ∉ [2600; 11500] W</td><td>Thiết bị không hỏng vận hành trong dải ~2960–10560 W (phân vị 1–99); dải cũ [3500; 9000] cắt vào vùng an toàn nên độ chính xác chỉ 0,17. Dải mới ôm vừa khít vùng an toàn → độ chính xác 0,79.</td></tr>
<tr><td>OSF</td><td>mòn × mômen &gt; {{12800; 13900; 14500}}</td><td>Xác suất hỏng tăng vọt đúng tại các mốc này theo từng loại (L: 0,88; M: 0,94; H: 0,93); ngưỡng tăng dần theo độ cứng sản phẩm, phù hợp cơ chế vật lý.</td></tr>
<tr><td>TWF</td><td>độ mòn dao &gt; 244 phút</td><td>Xác suất hỏng nhảy bậc từ 0,57 (mốc 240) lên 0,80 (mốc 244); dao không hỏng mòn tối đa chỉ tới 253.</td></tr>
</table>
<div class="note"><span class="t">Bằng chứng đóng góp (cô lập).</span> Khi bổ sung bốn đặc trưng này vào mô hình hồi
quy logistic cơ sở mà giữ nguyên mọi thành phần khác, F1 tăng từ 0,231 lên 0,716 và AUC-PR tăng từ 0,220 lên 0,645
(khoảng 2,9 lần). Hệ số tương quan điểm-nhị (point-biserial) của các đặc trưng biên đạt tối thiểu 0,18, riêng
<code>mon_twf</code> đạt 0,43 — vượt độ mòn dao thô (0,195). Với bộ ngưỡng khôi phục, độ chính xác gộp của luật tăng
từ 0,26 (cấu hình AI4I gốc) lên 0,81 trên A và giữ 0,82 trên B — xác nhận biên là bất biến, không phải tinh chỉnh
trên tập kiểm thử.</div>
...
<tr><td>Không hàm bản lề (<code>bien_overstrain</code>)</td><td>Không</td>
    ...
    quả không đổi (kiểm chứng thực nghiệm: giảm ngưỡng 100 hoặc 200 đơn vị, F1 giữ nguyên 0,783)</td></tr>
</table>
...
cây vượt trội (F1 tăng từ 0,716 lên 0,783). Nghiên cứu sử dụng ba mô hình cây theo ba cơ chế khác nhau nhằm tăng tính
đa dạng cho học kết hợp: Random Forest [5] (đóng bao, giảm phương sai), XGBoost [6] (tăng cường, giảm độ chệch) và
ExtraTrees [7] (ngẫu nhiên hóa ngưỡng, phương sai thấp hơn nữa). Tham số <code>max_features='sqrt'</code> được chọn thay vì giá trị
...
A nằm trong vùng B có xác suất xấp xỉ 1, khiến trọng số thô tăng đến khoảng 77 và một mẫu đơn lẻ chi phối toàn bộ
gradient; việc cắt ngưỡng giữ ổn định phương sai (chỉ 0,6% số mẫu chạm giới hạn).</p>
<div class="note"><span class="t">Về mức cải thiện nhỏ của hiệu chỉnh.</span> F1 giữ nguyên ở 0,783 và AUC-PR gần
như không đổi (0,675 → 0,671) là kết quả nhất quán với lý thuyết chứ không phải dấu hiệu thất bại. Drift-AUC tính
riêng trên các đặc trưng biên chỉ đạt 0,53 (Hình 7), nghĩa là trên không gian đặc trưng này, A và B gần như không
phân biệt được, do đó không còn nhiều dịch chuyển để hiệu chỉnh. Điều này khẳng định rằng thiết kế đặc trưng vật lý
đúng đắn làm giảm nhu cầu hiệu chỉnh dịch chuyển bằng phương pháp thống kê.</div>
...
IWV [3] nằm ở việc kỳ vọng hàm mất mát trên B có thể biểu diễn thành kỳ vọng có trọng số trên A:</p>
...
tốt. Cỡ mẫu hiệu dụng chỉ đạt 24,2% (do một số mẫu có trọng số lớn) khiến ước lượng IWV có nhiễu; vì vậy không nên
...
<tr><td>Bỏ phiếu — trung bình có trọng số (IWV chọn RF 0,25 / ExtraTrees 0,25 / XGB 0,5)</td><td>F1-IWV = 0,800; F1-B = 0,783</td>
    <td>Được chọn — đơn giản, ít tham số, ít quá khớp dịch chuyển</td></tr>
<tr><td>Xếp chồng — mô hình tổng hợp logistic (thiên về XGB, hệ số 2,6)</td><td>F1-IWV = 0,800; F1-B = 0,783</td>
    <td>Bị loại — dồn trọng số vào một mô hình cơ sở, kém ổn định hơn dưới dịch chuyển</td></tr>
</table>
<p>Hai phương án cho F1 trên B bằng nhau (0,783) và F1-IWV cũng bằng nhau (0,800), tức nằm trong sai số do cỡ mẫu
hiệu dụng thấp, nên được xem là gần tương đương. Trong trường hợp đó, phương án đơn giản và ổn định hơn
...
Phương pháp quét ngưỡng để cực đại hóa F1 trên thang IWV (mô phỏng B), thu được giá trị 0,585. Ngưỡng này gần với
ngưỡng tối ưu lý tưởng ước lượng trực tiếp trên B, cho thấy IWV đáng tin cậy; đồng thời đỉnh của đường
F1 theo ngưỡng khá phẳng quanh 0,585, cho thấy lựa chọn ngưỡng có tính ổn định.</p>
...
<div>
<table>
<caption>Bảng 9. Kết quả mô hình cuối cùng v6 (bỏ phiếu, ngưỡng 0,585) trên Dây chuyền B.</caption>
<tr><th>Độ đo</th><th>Giá trị</th></tr>
<tr><td>F1</td><td>0,783</td></tr>
<tr><td>AUC-ROC</td><td>0,872</td></tr>
<tr><td>AUC-PR</td><td>0,666</td></tr>
<tr><td>Precision</td><td>0,817</td></tr>
<tr><td>Recall</td><td>0,751</td></tr>
</table>
<p style="font-size:10pt;">Trong 477 thiết bị thực tế hỏng trên B, mô hình phát hiện 358 (dương thật) và bỏ sót 119
(âm giả); trong 438 cảnh báo phát ra, 80 là báo động giả (dương giả). Tỉ lệ cảnh báo đúng xấp xỉ bốn trên năm.</p>
</div>
...
<div class="cap"><b>Hình 6.</b> Chỉ số PSI: dịch chuyển tập trung ở biến thô; bốn đặc trưng biên có PSI xấp xỉ 0.</div></div>
<div class="fig"><img class="chart" src="{I['drift']}">
<div class="cap"><b>Hình 7.</b> Drift-AUC: đặc trưng biên không phân biệt được A và B (0,53 ≈ dự đoán ngẫu nhiên).</div></div>
</div>
...
<ul>
<li>Ước lượng điểm F1 = 0,783 với khoảng tin cậy 95% là [0,753; 0,811], ước lượng bằng phương pháp bootstrap [8].
Khoảng rộng (± 0,03) phản ánh việc chỉ có 477 mẫu hỏng trên B, khiến giá trị tuyệt đối của F1 kém chắc chắn; các
khoảng tin cậy của những phiên bản đầu bảng chồng lấn nhau, do đó không thể kết luận hơn kém dựa trên ước lượng điểm
đơn lẻ.</li>
<li>So sánh theo cặp trên cùng mẫu bootstrap cho kết quả tin cậy hơn: xác suất F1 của v6 lớn hơn F1 của XGBoost đạt
100%, với chênh lệch trung bình +0,015 và khoảng tin cậy 95% [+0,009; +0,022] nằm hoàn toàn ở phía dương. Điều này
cho thấy học kết hợp vượt trội nhất quán về hướng, tuy biên độ nhỏ (khoảng 1,5%).</li>
</ul>
...
<tr><td>Độ tin cậy của IWV bị giới hạn</td><td>Cỡ mẫu hiệu dụng 24,2% khiến ước lượng nhiễu; chênh lệch giữa bỏ phiếu và xếp chồng nằm trong sai số.</td></tr>
...
<tr><td>Trần hiệu năng của dữ liệu</td><td>Sau khi khôi phục luật, khoảng 25% ca hỏng vẫn không khớp bất kỳ luật nào — tương ứng lỗi ngẫu nhiên (kiểu <em>Random Failures</em> của AI4I [1]). Đây là nhiễu bất khả quy, giới hạn trần F1 ở mức xấp xỉ 0,78, khớp với hiện tượng mọi phiên bản từ v2 trở đi đều hội tụ về 0,783.</td></tr>
</table>

<!-- ================= 9. HUONG PHAT TRIEN ================= -->
<h2>9. Hướng phát triển</h2>
<div class="note"><span class="t">Cải tiến đã thực hiện.</span> Hướng "hiệu chỉnh lại hằng số ngưỡng" nêu ở phiên bản
trước đã được triển khai trong nghiên cứu này: bộ ngưỡng PWF/OSF được khôi phục từ Dây chuyền A và bổ sung luật TWF
(Mục 5.3), nâng độ chính xác gộp của luật từ 0,26 lên 0,81 và đưa F1 từ 0,781 lên 0,783. Các hướng còn lại:</div>
<ol>
...
<li>Áp dụng các kỹ thuật thích ứng miền nâng cao (CORAL, căn chỉnh đặc trưng đối kháng) để xử lý phần dịch
chuyển còn sót ở đuôi phân phối.</li>
...
trong báo cáo đều được luận giải kèm phương án thay thế và rủi ro tương ứng. Kết quả cuối cùng đạt F1 = 0,783 trên
Dây chuyền B, cải thiện 3,4 lần so với mô hình cơ sở, và phân tích cho thấy con số này đã chạm trần hiệu năng mà
dữ liệu cho phép.</p>
...
<div style="font-size:9.6pt; font-family:'Calibri','Segoe UI',sans-serif; line-height:1.45;">
<p style="margin:3px 0;">[1] S. Matzka, "Explainable Artificial Intelligence for Predictive Maintenance Applications,"
<em>2020 Third International Conference on Artificial Intelligence for Industries (AI4I)</em>, 2020, pp. 69–74.
(Bộ dữ liệu AI4I 2020 Predictive Maintenance, UCI Machine Learning Repository.)</p>
<p style="margin:3px 0;">[2] H. Shimodaira, "Improving predictive inference under covariate shift by weighting the log-likelihood
function," <em>Journal of Statistical Planning and Inference</em>, vol. 90, no. 2, pp. 227–244, 2000.</p>
<p style="margin:3px 0;">[3] M. Sugiyama, M. Krauledat, and K.-R. Müller, "Covariate shift adaptation by importance weighted cross
validation," <em>Journal of Machine Learning Research</em>, vol. 8, pp. 985–1005, 2007.</p>
<p style="margin:3px 0;">[4] J. Quiñonero-Candela, M. Sugiyama, A. Schwaighofer, and N. D. Lawrence (Eds.), <em>Dataset Shift in
Machine Learning</em>. Cambridge, MA: MIT Press, 2009.</p>
<p style="margin:3px 0;">[5] L. Breiman, "Random Forests," <em>Machine Learning</em>, vol. 45, no. 1, pp. 5–32, 2001.</p>
<p style="margin:3px 0;">[6] T. Chen and C. Guestrin, "XGBoost: A Scalable Tree Boosting System," <em>Proc. 22nd ACM SIGKDD</em>,
pp. 785–794, 2016.</p>
<p style="margin:3px 0;">[7] P. Geurts, D. Ernst, and L. Wehenkel, "Extremely Randomized Trees," <em>Machine Learning</em>,
vol. 63, no. 1, pp. 3–42, 2006.</p>
<p style="margin:3px 0;">[8] B. Efron and R. J. Tibshirani, <em>An Introduction to the Bootstrap</em>. New York: Chapman &amp;
Hall/CRC, 1993.</p>
<p style="margin:3px 0;">[9] N. V. Chawla, K. W. Bowyer, L. O. Hall, and W. P. Kegelmeyer, "SMOTE: Synthetic Minority Over-sampling
Technique," <em>Journal of Artificial Intelligence Research</em>, vol. 16, pp. 321–357, 2002.</p>
<p style="margin:3px 0;">[10] T. Saito and M. Rehmsmeier, "The Precision-Recall Plot Is More Informative than the ROC Plot When
Evaluating Binary Classifiers on Imbalanced Datasets," <em>PLoS ONE</em>, vol. 10, no. 3, e0118432, 2015.</p>
<p style="margin:3px 0;">[11] F. Pedregosa et al., "Scikit-learn: Machine Learning in Python," <em>Journal of Machine Learning
Research</em>, vol. 12, pp. 2825–2830, 2011.</p>
</div>
